\newcommand{\printportada}[6]{%
  \newgeometry{
    left=30mm,
    right=30mm,
    top=27.5mm,
    bottom=0mm,
    headheight=31.1pt
  }%
  
  \definecolor{AzulRect}{HTML}{064EA2}



  % ===== Portada 1 =====
  \begin{titlepage}
    \thispagestyle{empty}
    \begin{center}
        \includegraphics[scale=1]{FotosPlantilla/CEU-Logo.png}

        \vfill
        % Añadido \selectfont y corregido interlineado (24pt para letra de 20pt)
        {\fontspec{Arial}\fontsize{20pt}{21pt}\selectfont\bfseries TÍTULO \par} 
        %\vspace{0.3cm}
        {\fontspec{Arial}\fontsize{20pt}{21pt}\selectfont\bfseries #1 \par}

        \vfill\vfill\vfill

        {\fontspec{Arial}\fontsize{16pt}{20pt}\selectfont TRABAJO DE FIN DE MÁSTER \par}
        {\fontspec{Arial}\fontsize{16pt}{20pt}\selectfont EN CIBERSEGURIDAD \par}

        \vfill
        {\fontspec{Arial}\fontsize{16pt}{17pt}\selectfont #2 \par}
        \vfill
        {\fontspec{Arial}\fontsize{16pt}{17pt}\selectfont #4 \par} 

        \vfill\vfill\vfill
        \vspace{-0.5cm}
    \end{center}
    
    \begin{tikzpicture}[remember picture,overlay]
        \fill[AzulRect]
          ($(current page.south)+(0,0)$)
          ++(-10.5cm,-0.15cm) 
          rectangle ++(25cm,4.37cm);
    \end{tikzpicture}
  \end{titlepage}

  % ===== Portada 2 =====
  \begin{titlepage}
    \thispagestyle{empty}
    \begin{center}
        \includegraphics[scale=1]{FotosPlantilla/CEU-Logo.png}

        \vfill\vfill
        {\fontspec{Arial}\fontsize{20pt}{24pt}\selectfont\bfseries TÍTULO \par}
        \vspace{0.3cm}
        {\fontspec{Arial}\fontsize{20pt}{24pt}\selectfont\bfseries #5 \par}
        \vfill\vfill  

        {\fontspec{Arial}\fontsize{16pt}{20pt}\selectfont TRABAJO DE FIN DE MÁSTER \par}
        {\fontspec{Arial}\fontsize{16pt}{20pt}\selectfont EN CIBERSEGURIDAD \par}

        \vfill
        {\fontspec{Arial}\fontsize{16pt}{20pt}\selectfont #2 \par}
        \vspace{2cm}
        {\fontspec{Arial}\fontsize{14pt}{17pt}\selectfont #3 \par}
        \vfill
        {\fontspec{Arial}\fontsize{16pt}{20pt}\selectfont #4 \par} 
        \vfill
        \vfill
    \end{center}
  \end{titlepage}

  \restoregeometry
  \setcounter{page}{1}%
}